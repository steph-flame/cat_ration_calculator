...
//
// Intake kcal draws as a small vertical bar (hollow for a day flagged incomplete — the same
// "don't trust this one" convention TimelineChart uses for imputed points); daily median
// weight overlays as a thin dot-line on its own implicit scale (no shared axis with the bars —
// this is a glance-strip, not a second full chart). The viewed day gets a soft highlight pill;
// hover/focus gets a lighter one, so the strip visibly reads as clickable, not just decorative.
//
  ...
  return (
    <div className="mb-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-x-3 gap-y-1 mt-1">
        <span className="flex items-center gap-3 text-xs" style={{ color: C.sub }}>
      ...
      >
        <div style={{ position: "relative", width: totalW, height }}>
          <svg width={totalW} height={height} viewBox={`0 0 ${totalW} ${height}`} aria-hidden="true" style={{ display: "block" }}>
            ...
            {days.map((d, i) => d === hoverDay && d !== selected && (
              <rect key={`hov-${d}`} x={i * colW + 1} y={0} width={Math.max(colW - 2, 0)} height={height} rx={5} fill={C.spruceSoft} opacity="0.5" />
            ))}
            {days.map((d, i) => {
              const kcal = data[d]?.kcal;
